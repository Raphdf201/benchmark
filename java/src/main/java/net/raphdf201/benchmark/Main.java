...
public class Main {
    static void main(String[] args) {
        ...
        timeBenchmark("4. Matrix Multiply (500x500)", () ->
            matrixMultiply(500)
        );
    ...
    private static void timeBenchmark(String name, Supplier<Object> action) {
        final Object result;
        long startTime = System.currentTimeMillis();
        result = action.get();
        long elapsedTime = System.currentTimeMillis() - startTime;
        System.out.println(
            name + ": " + elapsedTime + "ms (result: " + result + ")"
        );
    }
    ...
    private static int mandelbrot(int n) {
        int count = 0;
    ...
    private static int binaryTrees(int n) {
    ...
    private record TreeNode(Optional<TreeNode> left, Optional<TreeNode> right) {
        public int check() {
